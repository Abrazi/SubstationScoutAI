...
// Find suspicious assignments where LHS begins with '(' or a literal or a function call
instrumented.forEach((ln, idx) => {
  const js = ln.split(';').slice(1).join(';').trim();
  // find assignment occurrences (first '=')
  const eqIndex = js.indexOf('=');
  if (eqIndex <= 0) return;
  const lhs = js.slice(0, eqIndex).trim();
  if (/^[('"\d]/.test(lhs) || /\)\s*$/.test(lhs)) {
    console.log(`Suspicious LHS at source line ${idx + 1}: "${lhs}"  -> ${js}`);
  }
});
